...
export async function makeRequest() {
	...
	console.log(
		chalk.green("🔎 Getting data from Shodan's API, please wait...")
	);
	const parsed = await got(
		"https://api.shodan.io/shodan/host/search?key=" +
			process.env.GRIEFFICIENT_API_KEY +
			"&query=Minecraft",
		{
	...
	for (var i = 0; i < thejson.matches.length; i++) {
		...
		console.log(
			chalk.blue(matches.ip_str + ":" + matches.port) +
				chalk.cyan(" (" + matches.version + ")")
		);
	}
	console.log(
		"👉 " +
			chalk.bold.cyan(Object.keys(thejson.matches).length) +
			" results shown out of " +
			chalk.bold.cyan(total_results.total) +
			" possible results (blame Shodan)"
	);
}
